...
file_location = get_package_share_directory('yolov5_ros')
...
from big_arm_interfaces.msg import CameraBoundingBoxes, CameraBox, ErrorReport
...
class yolov5_fcamera1_ros(Node):
    def __init__(self):
        super().__init__('yolov5_fcamera1_ros')
        trained_weights = '27SEP_Best.pt' 
        self.declare_parameter('using_sim', True)
        self.declare_parameter('weights', os.path.join(file_location, 'config', trained_weights))
        self.declare_parameter('data', os.path.join(file_location, 'data', 'coco128.yaml'))
        ...
        # Initialize cv_bridge to connect ros2 images to opencv
        self.bridge = CvBridge()

        self.pub_fcamera1_bbox = self.create_publisher(CameraBoundingBoxes, 'yolov5/f_camera1/bounding_boxes', 10)
        self.pub_fcamera1_image = self.create_publisher(Image, 'yolov5/f_camera1/image', 10)
        self.pub_error_msgs = self.create_publisher(ErrorReport, '/vehicle/errors', 10)

        if (not self.using_sim): # If we are not using sim, subscripe to CompressedImage instead of Image interface
            self.sub_f_camera1 = self.create_subscription(CompressedImage, '/camera/front_left_center/image/compressed', self.image_callback, 10)
        else:
            self.sub_f_camera1 = self.create_subscription(Image, '/camera/front_left_center/image/compressed', self.image_callback, 10)


        self.yolov5_fcamera1 = yolov5_demo(self.weights,
                                self.data,
                                self.imagez_height,
                                self.imagez_width,
                                self.conf_thres,
        ...
        self.yolov5_fcamera2 = yolov5_demo(self.weights,
                        self.data,
                        self.imagez_height,
                        self.imagez_width,
                        self.conf_thres,
    ...
    def yolovFive2bboxes_msgs(self, bboxes:list, scores:list, cls:list, img_header:Header):
        bboxes_msg = CameraBoundingBoxes()
    ...
    def image_callback(self, image): # Callback fcamera1
        #bridge.imgmsg_to_cv2(image_message, desired_encoding='passthrough')

        try:
            image_raw = self.bridge.compressed_imgmsg_to_cv2(image)
        except:
            error_msg = ErrorReport()
            error_msg.description = "Unable to get image from fcam1"
            error_msg.severity = 1
            error_msg.module = "Perception"
            error_msg.origin = "yolov5_fcamera1_ros"
            error_msg.lifetime = 1
            self.pub_error_msgs.publish(error_msg)
    
        class_list, confidence_list, x_min_list, y_min_list, x_max_list, y_max_list = self.yolov5_fcamera1.image_callback(image_raw)
        ...
        self.pub_fcamera1_bbox.publish(msg)

        image_message = self.bridge.cv2_to_imgmsg(image_raw, encoding="bgr8")
        self.pub_fcamera1_image.publish(image_message)

        #print("start fcamera1 ==================")
...
def ros_main(args=None):
    ...
    yolov5_node = yolov5_fcamera1_ros()
